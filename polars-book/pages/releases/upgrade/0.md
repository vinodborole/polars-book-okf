...
resource: https://docs.pola.rs/releases/upgrade/0.20
...
`Datetime` which *will* be instantiated in many cases.
...
may result in faulty logic, as Python will evaluate the *method* to `True`. For example,
...
- The logic for determining the return data type has changed. You may want to specify
   `return_dtype`to override the inferred data type, or take advantage of the new function signature (separate`old`and`new`parameters) to influence the return type.
- The previous workaround for referencing other columns as default by using a struct column no longer works. It now simply works as expected, no workaround needed.
...
1. Source page: https://docs.pola.rs/releases/upgrade/0.20
